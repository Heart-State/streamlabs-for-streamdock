...
const $propEvent = {
    ...
    didReceiveSettings({ settings }) {
        if ('volumeState' in settings) {
            $dom.volumeState.value = settings.volumeState;
        }
    },
...
// 切换状态
$dom.volumeState.on('change', (event) => {
    $websocket.sendToPlugin({ "updateVolumeState": event.target.value });
    $settings.volumeState = event.target.value;
    // 根据选中的值切换状态
})
