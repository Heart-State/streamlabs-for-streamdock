...
function viewLoad() {
    ...
    fetch('http://localhost:3056/getProfilesId', options)
        .then(response => response.json())
        .then(data => {
            data.profiles.forEach((profile) => {
                $dom.profilesId.insertAdjacentHTML('beforeend', `<option value="${profile.id}">${profile.name}</option>`);
            })
            $dom.profilesId.value = $settings.profilesId = data.active_profile;
        })
...
const $propEvent = {
    ...
    didReceiveSettings({ settings }) {
        if ('profilesId' in settings) {
            $dom.profilesId.value = settings.profilesId;
        }
...
$dom.user.on('click', function () {
    ...
    // window.open("http://localhost:3001/validateOne?code=" + tempToken, Date.now(), 'width=400,height=300');
});

// 切换状态
$dom.profilesId.on('change', (event) => {
    $settings.profilesId = event.target.value;
})
